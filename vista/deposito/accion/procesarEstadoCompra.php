        ...
        if ($idCompraEstadoTipoAnterior == 4) {
            ...
            if ($compraEstadoTipo) {
                ...
                if ($objCompraEstado->modificacion($paramEstadoCompraAnterior) && $objCompraEstado->alta($paramEstadoCompraNuevo)) {
                    ...
                    /*
                    // Enviar correo de notificación (según el estado de la compra)
                    $estadoCompraTipoDescripcion = $compraEstadoTipo->getCetDescripcion();
                    $resultadoCorreo = enviarCorreo($estadoCompra->getObjCompra()->getObjUsuario()->getUsMail(), $estadoCompraTipoDescripcion);
                    ...
                    if ($resultadoCorreo) {
                        ...
                        $response['success'] = true; // Aunque el correo falló, la modificación fue exitosa
                        $response['message'] = 'El estado de la compra fue modificado, pero hubo un problema al enviar el correo.';
                    }
                    */
                    
                    // Enviar correo de notificación (según el estado de la compra)
                    $resultadoCorreo = enviarCorreoConPDF(
                        $estadoCompra->getObjCompra()->getObjUsuario()->getUsMail(),
                        $idCompra,
                        $compraEstadoTipo,
                        $objCompraItem
                    );
